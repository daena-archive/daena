...
This document is the architecture for AI in Daena Archive. It governs provider
integration, prompting, context assembly, retrieval-augmented generation (RAG),
plugin access, privacy, and user experience. Phased delivery is in
[`plans/ai.md`](../plans/ai.md).
...
- [`PLUGIN_PLATFORM_PLAN.md`](../plans/PLUGIN_PLATFORM_PLAN.md) defines plugin identity,
  isolation, capabilities, broker authorization, and public-contract rules.
- [`STORAGE.md`](../storage/STORAGE.md) defines the database-first runtime, portable
...
Delivery phases are in [`plans/ai.md`](../plans/ai.md). The provider, consent, capability, indexing, and verification rules in this document are authoritative.
...
Legacy settings are rejected rather than migrated. Host-derived `AiCaller` propagation, a bounded
...
limits. V1 implements local text-to-image through a separately configured
ComfyUI provider; there is no local-to-remote fallback. A project visual profile
is portable author-owned content, not provider configuration. A later phase may
add an optional, provider-neutral `visualProfile` object
...
Accepted-image provenance records the final user-reviewed prompt, optional
negative prompt, prompt-generation method, text provider/model when used, image
provider/model, seed, dimensions, bounded generation parameters, selected
context entity IDs, generation timestamp, and source request ID. This metadata
is attached to the canonical asset and therefore leaves temporary storage only
after explicit acceptance.

Vision analysis, reference-image conditioning, image embeddings, and editing are
later work. They require explicit asset-read scope and separate remote
...
### 14.2 Plugin-generated character biography
...
### 14.3 Consistency analysis with RAG
...
The intended AI platform is a reusable host service rather than a
