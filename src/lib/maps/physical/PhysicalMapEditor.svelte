<script lang="ts">
  ...
  import { DEFAULT_VECTOR_LAYER_STYLE, type VectorFeatureCollection, type VectorLayerDefinition } from "../native-vector/types";
  ...
  const layers: VectorLayerDefinition[] = [
    {
      id: "base",
      kind: "vector",
      name: "Physical base",
...
{:else}
  <section class="native-vector-editor physical-map-editor" aria-label="Generate physical map">
    <header>
      <div><span>PHYSICAL WORLD</span><strong>{busy ? `${status?.stage ?? "Starting"}…` : "One world, one preview"}</strong></div>
    ...
    {#if status}<p class="physical-map-progress" role="status">{status.stage} · {status.completed}/{status.total}</p>{/if}
