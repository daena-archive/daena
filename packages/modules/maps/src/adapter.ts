...
/** Provider selection stays at this boundary. Callers select by the durable
 * descriptor ID and never reach into a different provider's implementation. */
export function selectProviderAdapter(
  providerId: string,
  adapters: ReadonlyMap<string, MapProviderAdapter>,
): MapProviderAdapter {
  const adapter = adapters.get(providerId);
  if (!adapter) throw new Error(`unsupported map provider: ${providerId}`);
  return adapter;
}
