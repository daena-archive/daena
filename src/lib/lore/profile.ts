...
export function profileValidationErrors(input: unknown): string[] {
  ...
  if (document.presetOrigin !== undefined && document.presetOrigin !== "custom") {
    errors.push("Only Custom profiles can be saved");
  }
...
export function parseProfile(input: unknown): ProfileDocument {
  ...
  return {
    ...
    ...(document.presetOrigin === "custom" ? { presetOrigin: "custom" as const } : {}),
...
export function componentHasValue(component: ProfileComponent): boolean {
  ...
  if (value.type === "resource") {
    return value.current !== null || value.max !== null || Boolean(value.unit?.trim());
  }
...
export function formatProfileValue(component: ProfileComponent): string {
  const value = component.value;
  if (value.type === "resource") {
    if (value.current === null && value.max === null && !value.unit?.trim()) return "";
