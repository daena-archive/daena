...
const CHROME_CONTRAST_PAIRS = [["rail-text", "rail-bg"]];
...
export function parseThemeColor(value) {
    ...
    if (![...hex].every((ch) => /[0-9A-Fa-f]/.test(ch)))
        throw new Error(`invalid theme color: ${value}`);
...
export function mergeThemeTokens(base, overlay) {
    const merged = { ...base };
    for (const [key, value] of Object.entries(overlay)) {
        if (!TOKEN_IDS.has(key))
            throw new Error(`unknown theme token: ${key}`);
        merged[key] = parseThemeColor(value);
    }
    return merged;
}
export function resolveThemeTokens(mode, overlay) {
...
export function validateThemePack(pack) {
    ...
    if (typeof pack.name !== "string" || !pack.name.trim())
        errors.push(`theme ${pack.id} name is required`);
    else if ([...pack.name].length > THEME_PACK_NAME_MAX_CHARS)
        errors.push(`theme ${pack.id} name is too long`);
    if (!isTokenMap(pack.tokens.light))
        errors.push(`theme ${pack.id} light tokens must be an object`);
    else {
        try {
    ...
    if (!isTokenMap(pack.tokens.dark))
        errors.push(`theme ${pack.id} dark tokens must be an object`);
    else {
        try {
...
function checkContrastPairs(tokens, pairs, minimum) {
    for (const [foreground, background] of pairs) {
        const fg = tokens[foreground];
        const bg = tokens[background];
        if (!fg)
            throw new Error(`missing theme token: ${foreground}`);
        if (!bg)
            throw new Error(`missing theme token: ${background}`);
        const ratio = contrastRatio(fg, bg);
        if (ratio + Number.EPSILON < minimum) {
            throw new Error(`theme contrast ${foreground}/${background} is ${ratio.toFixed(2)}, expected at least ${minimum}`);
        }
    }
}
...
export function applyPluginAppearance(appearance, root) {
    ...
    for (const id of THEME_TOKEN_IDS) {
        ...
        if (typeof value === "string" && TOKEN_HEX.test(value))
            root.style.setProperty(`--${id}`, value);
        else
            root.style.removeProperty(`--${id}`);
    }
}
export function hostHasFeature(features, feature) {
