...
assert.deepEqual(
  selectedPhysicalFeatures(mixed, "rivers", ["lake-a"]).map((item) => item.id),
  [],
);
...
const plan = buildPhysicalDetachPlan({
  ...
  epochOffsetYears: -250,
  scope: "selected",
  selectedIds: ["derived-a"],
...
assert.deepEqual(plan.copies[0].properties.daena.custom, {
  ...
  detachedFromLayerId: "rivers",
  detachedFromFeatureId: "derived-a",
  detachedAtEpochYears: -250,
});
...
const layerScopePlan = buildPhysicalDetachPlan({
  ...
  epochOffsetYears: 0,
  scope: "layer",
  newId: () => layerScopeIds.shift() ?? "unexpected-id",
});
...
assert.deepEqual(
  layerScopePlan.copies.map((item) => item.id),
  ["copy-1", "copy-2"],
);
...
const totalPositionsPlan = buildPhysicalDetachPlan({
  collection: derived,
  document: totalPositionsDocument,
  sourceLayer: physical,
  epochOffsetYears: 0,
  scope: "selected",
  selectedIds: ["derived-a"],
...
assert.equal(
  totalPositionsPlan.message,
  `Detaching would exceed the authored position limit of ${VECTOR_MAX_POSITIONS}.`,
);
...
const authoredOnlyPlan = buildPhysicalDetachPlan({
  collection: derived,
  document: authoredOnlyDocument,
  sourceLayer: authoredOnlyLayer,
  epochOffsetYears: 0,
  scope: "layer",
});
assert.equal(
...
assert.equal(
  stack.document.layers.some((layer) => layer.id === "target-layer"),
  true,
);
...
assert.equal(
  stack.document.layers.some((layer) => layer.id === "target-layer"),
  true,
  "redo restores the same target layer",
);
...
hiddenStack.apply(
  detachPhysicalFeaturesCommand({
    ...
    targetLayer: layerScopePlan.targetLayer,
    copies: layerScopePlan.copies,
  }),
);
