...
//      plugin-error-v1.json, capability-registry-v1.json, and
...
//      Also emit `static/theme-init.js` from the theme-token catalog.
//
...
// the `DAENA_SCHEMA_DIR` / `DAENA_GENERATED_TS` / `DAENA_THEME_INIT` overrides:
// when a schema dir is supplied the Rust bin is skipped (the caller has
// already generated the schemas) and conversion runs against the supplied
// schemas, writing `generated.ts` and `theme-init.js` to the supplied paths.
...
const themeInitPath = process.env.DAENA_THEME_INIT ?? "static/theme-init.js";
...
const themeSchema = await readJson("theme-tokens-v1.json");
...
const themeTokenIds = themeSchema.tokenIds;
const builtinThemeTokens = themeSchema.builtin;
if (!Array.isArray(themeTokenIds) || themeTokenIds.length === 0)
  throw new Error("theme-tokens schema must declare tokenIds");
if (!builtinThemeTokens?.light || !builtinThemeTokens?.dark)
  throw new Error("theme-tokens schema must declare builtin light and dark maps");
for (const mode of ["light", "dark"]) {
  const missing = themeTokenIds.filter((id) => typeof builtinThemeTokens[mode][id] !== "string");
  if (missing.length > 0) throw new Error(`theme-tokens ${mode} map missing ${missing.join(", ")}`);
  const extra = Object.keys(builtinThemeTokens[mode]).filter((id) => !themeTokenIds.includes(id));
  if (extra.length > 0) throw new Error(`theme-tokens ${mode} map has unknown keys ${extra.join(", ")}`);
}
manifestLines.push(`export const THEME_TOKEN_IDS = ${JSON.stringify(themeTokenIds)} as const;`);
manifestLines.push(`export type ThemeTokenId = typeof THEME_TOKEN_IDS[number];`);
manifestLines.push(
  `export const BUILTIN_THEME_TOKENS = ${JSON.stringify({
    light: Object.fromEntries(themeTokenIds.map((id) => [id, builtinThemeTokens.light[id]])),
    dark: Object.fromEntries(themeTokenIds.map((id) => [id, builtinThemeTokens.dark[id]])),
  })} as const;`,
);
...
const themeInit = `/* generated by scripts/gen-plugin-contract.mjs — do not edit */
(() => {
  const LIGHT = ${jsString(Object.fromEntries(themeTokenIds.map((id) => [id, builtinThemeTokens.light[id]])))};
  const DARK = ${jsString(Object.fromEntries(themeTokenIds.map((id) => [id, builtinThemeTokens.dark[id]])))};
  const IDS = ${jsString(themeTokenIds)};
...
`;
await writeFile(resolve(root, themeInitPath), themeInit);
console.log(`wrote ${themeInitPath}`);
