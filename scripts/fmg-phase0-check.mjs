...
function fail(message) { console.error(`FMG Phase 0 check failed: ${message}`); process.exitCode = 1; }
if (!process.argv[2]) { fail("usage: node scripts/fmg-phase0-check.mjs /path/to/pinned-fmg"); process.exit(2); }
if (!existsSync(sourceRoot)) fail(`source directory does not exist: ${sourceRoot}`);
if (existsSync(sourceRoot)) {
  let commit;
  try { commit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim(); } catch { fail("source is not a git checkout"); }
  if (commit && commit !== expectedCommit) fail(`expected commit ${expectedCommit}, got ${commit}`);
  const license = join(sourceRoot, "LICENSE");
  if (!existsSync(license) || !readFileSync(license, "utf8").includes("MIT License")) fail("FMG MIT license notice is missing");
  ...
  for (const path of requiredStaticPaths) if (!existsSync(join(sourceRoot, path))) fail(`required FMG path is missing: ${path}`);
}
