...
<style>
  ...
  .projection-view-header .quiet-button { padding: 8px 10px; border: 1px solid #ded8cd; border-radius: 8px; background: var(--surface); color: var(--ink-soft); font: 500 11px/1.2 var(--font-body, Inter, ui-sans-serif, system-ui, sans-serif); box-shadow: 0 1px 2px rgba(48,45,38,.05); cursor: pointer; transition: background .16s ease, border-color .16s ease, box-shadow .16s ease, color .16s ease, transform .16s ease; }
  .projection-view-header .quiet-button:hover { border-color: #cbbda9; background: var(--surface-muted); color: var(--ink); box-shadow: 0 3px 8px rgba(48,45,38,.08); transform: translateY(-1px); }
  .projection-view-header .quiet-button:active { box-shadow: 0 1px 2px rgba(48,45,38,.05); transform: translateY(1px); }
  .projection-view-header .quiet-button:focus-visible { outline: 3px solid rgba(180,119,63,.24); outline-offset: 2px; }
