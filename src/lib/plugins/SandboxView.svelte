<script lang="ts">
  ...
  let {
    ...
    viewId,
    title,
  }: { pluginId: string; viewId?: string; title: string } = $props();
...
<style>
  .sandbox-view-shell { display: flex; min-height: 0; height: auto; flex: 1 1 auto; flex-direction: column; }
  ...
  @media (max-width: 760px) { .sandbox-view-loading, .sandbox-view-error { margin: 20px 17px; } }
</style>
