...
export function validatePluginManifest(manifest) {
    ...
    if (value.importers !== undefined && !Array.isArray(value.importers))
    ...
    if (Array.isArray(value.importers)) {
        if (value.importers.length > 8)
            errors.push("too many importer contributions");
        ...
        for (const importer of value.importers) {
            ...
            if (reserved.has(id))
                errors.push(`importer ${id} uses a reserved id`);
            if (!isPluginIdentifier(id) || importerIds.has(id))
                errors.push(`invalid or duplicate importer: ${id}`);
            else
                importerIds.add(id);
            ...
            if (typeof importer.description !== "string" ||
            ...
            if (!Array.isArray(importer.extensions) ||
                ...
                importer.extensions.some((extension) => typeof extension !== "string" ||
                    extension.length === 0 ||
                    extension.length > 16 ||
                    extension.startsWith(".") ||
                    !/^[a-z0-9]+$/.test(extension)))
                errors.push(`importer ${id} extensions are invalid`);
            if (importer.mimeTypes !== undefined &&
            ...
            if (!capabilities.includes(capability))
                errors.push(`importer ${id} requires capability ${capability}`);
