# FMG Phase 0 spike record

- Status: pinned source vendored and packaged; rendered child-webview acceptance remains a desktop check.
- FMG source: `Azgaar/Fantasy-Map-Generator` tag `v1.119`
- Pinned commit: `3430c22204f60baa412d4657ca2f9d00c270eda9`
- Local checkout: `third_party/fantasy-map-generator/` (gitignored; see
  [`third_party/README.md`](../../third_party/README.md))
- License: MIT; retain `LICENSE` verbatim in every bundled package.
- Fixture: upstream `tests/fixtures/demo.map`, SHA-256
  `f1ab797dcf7e2a383b0753e965342c374e86c6ce1db666ea2a4abc8122504799`
- Daena vendor archive: `src-tauri/plugin-assets/maps/fmg-v1.119.zip` (the
  archive contains the complete static tree and `FMG-LICENSE`).

The archive is generated by the FMG transform script, not patched at request
time by Rust:

```sh
node scripts/patch-fmg-for-daena.mjs third_party/fantasy-map-generator \
  src-tauri/plugin-assets/maps/fmg-v1.119.zip
```

The script transforms the built `dist/` tree, including the bridge ordering,
stylesheet links, inline-style bootstrap, and provider-startup fallback, then
packages the complete static tree.

The generated archive is intentionally gitignored. `npm run prepare:maps` and
the Tauri Cargo build both verify its SHA-256 and accept it when it matches the
hash in `docs/maps/fmg-v1.119-vendor.json`. If it is missing or stale,
`prepare:maps` regenerates it from `third_party/fantasy-map-generator` when
present, or from `DAENA_FMG_SOURCE`.

## Map source persistence (pragmatic path)

Keep FMG `.map` bytes as opaque native assets under `assets/maps/` on the map
entity. Do not parse or rewrite them in Rust.

1. **Create** — `create_map` makes a `daena.maps:map` entity with
   `sourceAssetId: null`.
2. **First save** — the FMG bridge calls `prepareMapData()`, uploads through
   `maps.asset.create.begin/commit`, then `field.set`s the descriptor
   `sourceAssetId` to the new asset. Canonical bytes land at
   `assets/maps/<uuid>-map.map`.
3. **Later saves** — `asset.replace.begin/commit` with the observed revision.
4. **Load** — `asset.read.begin` streams those bytes back; the bridge calls
   FMG `uploadMap(...)`.

That path is already the product contract: project files remain canonical,
`.daena/` stays disposable, and FMG stays usable outside Daena on the same
`.map` file.

## Reproducible build observation

From a clean checkout at the pinned commit, Node `v26.6.0` and npm `11.18.0`:

```sh
npm ci --no-audit --no-fund
npm run build
```

Do not use `npm ci --ignore-scripts` here: esbuild needs its platform optional
dependency (`@esbuild/darwin-arm64` on Apple Silicon). The build completed with
Vite `7.3.2`, producing a 27 MiB `dist/` tree. The fixture is 4.1 MiB. The build
warns that many non-module scripts and styles are left as runtime-relative
URLs; `dist/` must therefore be packaged as a complete static tree, not just
Vite's hashed files. These are source/build observations, not yet host limits
or performance budgets.

`npm ci --offline` before an initial dependency acquisition failed because the
local cache lacked `ws-8.19.0.tgz`. The delivery build needs a vendored package
cache or a locked builder image; a first-install network is not an acceptable
runtime dependency.

## Runtime and adapter boundary

The spike must use a Maps-owned adapter with exactly these operations:

| Intent | Pinned upstream hook | Status |
| --- | --- | --- |
| load source bytes | `uploadMap(file)` in `public/modules/io/load.js` | wrapper required |
| serialize source bytes | `prepareMapData()` in `public/modules/io/save.js` | wrapper required |
| focus coordinates | `zoomTo(x, y, z, duration)` in `public/main.js` | wrapper required |
| select a burg | `pack.burgs[id]` + `editBurg(id)` | adapter-only selector |
| select a state | `pack.states[id]` + state editor | adapter-only selector |
| select a river | `pack.rivers[id]` + `editRiver(id)` | adapter-only selector |

These upstream functions and the `pack` global are not a public API. The
wrapper must expose a narrow module API and prevent Daena code from accessing
them directly. It must replace browser download, IndexedDB autosave, Dropbox,
AI, service-worker registration, and URL-loading paths with inert or
host-mediated behavior. The small maintained patch is expected to export the
adapter hooks and add a save callback; no patch has been accepted yet.

Stable initial selectors are `burg`, `state`, and `river` numeric `i` values,
each paired with a normalized fallback point. They become unresolved when the
corresponding record is removed, when an upstream conversion changes its `i`,
or after topology-changing regeneration/import operations. Never retarget by
name, ordinal position, or proximity alone.

## Automated spike check

Run `node scripts/fmg-phase0-check.mjs /path/to/pinned-fmg`. It rejects an
unpinned source version, a partial static output, and a non-representative
fixture, then records fixture/compressed sizes. This is intentionally separate
from the public plugin conformance suite.

## Remaining exit-gate evidence

The source, offline build, archive, protocol path, editor lifecycle, and
conflict/recovery flows are now implemented and covered by automated checks
(Rust: `bundled_maps_shell_is_deterministic_and_provider_fail_closed`,
`recovery_uploads_are_bound_chunk_ordered_and_hash_verified`,
`map_recovery_copies_are_canonical_listed_newest_first_and_restored`,
`map_recovery_copies_require_map_entities_and_reject_traversal`, seed example
with empty-source map; Deno: `maps-adapter.test.ts` controller lifecycle +
bridge invariants; `deno task check` / `build` / `check:plugin-contract`).

A packaged Tauri development build still has to prove, in a real child webview:
offline boot; load/edit/serialize/reload of the fixture; selection and viewport
focus; no network or Tauri globals; cleanup on close (no webview/session/staging
left behind on workspace switch); and representative load/save time, memory,
and compressed-input expansion measurements. Conflict/recovery and save-state
UI also need an interactive pass per platform (see `docs/PHASE3_MAPS_PLAN.md`).

The candidate contract for review is
[`maps-contract-v1.schema.json`](./maps-contract-v1.schema.json). It is a
documentation-only draft and must not be copied into `schemas/` or a plugin
manifest before review.
