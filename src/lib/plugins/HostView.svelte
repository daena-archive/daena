...
async function saveField(
  component: Extract<ViewComponent, { type: "field-form" }>,
      ...
      {#each view.components ?? [] as component (component.id)}
          ...
          <article class="host-view-card">
            ...
            {#if selected}
              <dl class="host-view-detail">
                ...
                <div>
                  <dt>Type</dt>
                  <dd>{selected.entity_type ?? "Unspecified"}</dd>
                </div>
                <div>
                  <dt>Updated</dt>
                ...
                {#each component.fields as key (key)}
                  ...
                  {#if field}
                    <label>
                      ...
                      {#if field.type === "enum"}
                        <select
                          disabled={!component.editable || saving === `${component.id}:${field.key}`}
                          value={String(inputValue(field.key))}
                          onchange={(event) =>
                            void saveField(component, field, (event.currentTarget as HTMLSelectElement).value)}>
                      ...
                      {:else if field.type === "boolean"}
                        <input
                          type="checkbox"
                          disabled={!component.editable || saving === `${component.id}:${field.key}`}
                          checked={inputValue(field.key) === true}
                          onchange={(event) =>
                            void saveField(component, field, (event.currentTarget as HTMLInputElement).checked)} />
                      {:else}
                        <input
                          ...
                          value={String(inputValue(field.key))}
                          onchange={(event) =>
                            void saveField(component, field, (event.currentTarget as HTMLInputElement).value)} />
                      {/if}
                    </label>
                  {/if}
                {/each}
        ...
        {:else if component.type === "button"}
          <div class="host-view-action">
            <button
              type="button"
              class="host-view-button"
              disabled={commandAction(component.command) !== "refresh-view"}
              onclick={() => void invokeCommand(component.command)}>{component.label}</button>
          </div>
        {/if}
      {/each}
    </div>
...
.host-view-heading h1 {
  margin: 7px 0 0;
  font: 500 34px/1.05 var(--font-display);
}
.host-view-badge {
  padding: 6px 9px;
...
.host-view-card {
  ...
  box-shadow: 0 8px 24px rgba(62, 42, 25, 0.05);
}
.host-view-card-heading {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.host-view-card-heading h2 {
  margin: 0;
  font: 500 21px var(--font-display);
}
.host-view-card-heading > span {
  color: var(--ink-faint);
  font-size: 12px;
}
.host-view-list {
...
.host-view-list button.selected {
  color: var(--accent-dark);
}
.host-view-list button span {
  color: var(--ink-faint);
}
.host-view-detail {
  display: grid;
...
.host-view-form label {
  display: grid;
  gap: 6px;
  color: var(--ink-soft);
  font-size: 11px;
}
.host-view-form input,
.host-view-form select {
  min-height: 34px;
  padding: 7px 9px;
  border: 1px solid var(--line);
  border-radius: 7px;
...
.host-view-form input[type="checkbox"] {
  width: 16px;
  min-height: 16px;
}
.host-view-action {
